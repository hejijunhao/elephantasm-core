...
def run_migrations_offline() -> None:
    ...
    # Use direct connection URL for offline mode
...
def run_migrations_online() -> None:
    ...
    # Transform plain postgresql:// URL to sync psycopg driver format
    database_url = settings.MIGRATION_DATABASE_URL.replace("postgresql://", "postgresql+psycopg://")

    # Create synchronous engine for migrations (matches Marlin Hybrid pattern)
    connectable = create_engine(
        database_url,
        poolclass=pool.NullPool,
        connect_args={
            "options": "-c search_path=public"  # Force migrations to public schema
        }
    )
