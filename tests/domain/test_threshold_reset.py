...
def test_baseline_uses_last_synthesis_check_at_when_most_recent(db_session):
...
def test_baseline_prefers_last_memory_if_more_recent(db_session):
...
def test_baseline_falls_back_to_anima_created_at(db_session):
...
def test_threshold_reset_prevents_infinite_accumulation(db_session):
...
def test_baseline_timezone_aware(db_session):
