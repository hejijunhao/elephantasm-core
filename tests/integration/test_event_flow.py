...
def test_anima(client: TestClient) -> dict:
    """Create a test anima for event tests."""
    response = client.post("/api/animas", json={"name": "Event Test Anima"})
...
class TestCreateEvent:
    ...
    def test_create_event_success(self, client: TestClient, test_anima: dict):
        ...
        assert data["event_type"] == "message_in"
...
class TestListEvents:
    ...
    def test_list_events_by_type(self, client: TestClient, test_anima: dict):
        ...
        response = client.get(
            f"/api/events?anima_id={test_anima['id']}&event_type=message_in"
        )
        assert response.status_code == 200
        data = response.json()
        assert all(e["event_type"] == "message_in" for e in data)
