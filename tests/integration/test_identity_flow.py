...
@pytest.fixture
def test_anima(client: TestClient) -> dict:
    """Create a test anima for identity tests."""
    response = client.post("/api/animas", json={"name": "Identity Test Anima"})
...
class TestCreateIdentity:
    ...
    def test_create_identity_with_personality(self, client: TestClient):
        """Create identity with personality type."""
        # Create anima first
        anima = client.post("/api/animas", json={"name": "Personality Anima"}).json()

        response = client.post(
            f"/api/identities?anima_id={anima['id']}",
            json={
    ...
    def test_create_identity_conflict(self, client: TestClient):
        """Create identity twice for same anima returns 409."""
        # Create anima
        anima = client.post("/api/animas", json={"name": "Conflict Anima"}).json()

        # First identity
        resp1 = client.post(f"/api/identities?anima_id={anima['id']}", json={})
        assert resp1.status_code == 201

        # Second identity - should conflict
        resp2 = client.post(f"/api/identities?anima_id={anima['id']}", json={})
