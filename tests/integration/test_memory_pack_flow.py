...
@pytest.fixture
def test_anima(test_context: dict) -> dict:
    """Create a test anima via domain ops (bypasses subscription limit check)."""
    admin = get_admin_session()
    try:
        anima = AnimaOperations.create(
            admin,
            AnimaCreate(name="PackTest Anima"),
            user_id=test_context["user_id"],
        )
        admin.commit()
        return {"id": str(anima.id), "name": anima.name}
    finally:
        admin.close()
