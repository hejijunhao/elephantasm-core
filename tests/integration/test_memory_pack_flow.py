...
class TestGetLatestMemoryPack:
    ...
    def test_latest_compile_on_miss(self, client: TestClient, test_anima: dict):
        """Compiles a pack on-the-fly when no cached packs exist (compile-on-miss)."""
        response = client.get(f"/api/animas/{test_anima['id']}/memory-packs/latest")
        assert response.status_code == 200
        data = response.json()
        assert data is not None
        assert data["anima_id"] == test_anima["id"]
        assert data["preset_name"] == "conversational"
