...
def test_anima(client: TestClient) -> dict:
    """Create a test anima for SDK tests."""
    response = client.post("/api/animas", json={"name": "SDK Test Anima"})
...
class TestAPIKeyAuthentication:
    ...
    def test_sdk_auth_with_valid_key(self, client: TestClient, test_anima: dict):
        ...
        api_key = key_data.get("key") or key_data.get("api_key") or key_data.get("secret")
...
class TestInvalidAPIKey:
    ...
    def test_invalid_api_key_format(self, unauthenticated_client: TestClient):
        """Invalid API key format - RLS returns empty list (not 401)."""
        # Try with bad format - RLS filters to empty, doesn't reject
        response = unauthenticated_client.get(
            "/api/animas",
            headers={"Authorization": "Bearer invalid_key_format"}
        )
        # RLS-protected endpoints return 200 with empty data when no valid user
        assert response.status_code == 200
        assert response.json() == []

    def test_missing_auth_header(self, unauthenticated_client: TestClient):
        """Missing auth header - RLS returns empty list."""
        response = unauthenticated_client.get("/api/animas")
        # RLS filters to empty result, doesn't require explicit auth
        assert response.status_code == 200
        assert response.json() == []
