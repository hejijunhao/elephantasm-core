...
def _snapshot_org_state(session: Session, org_id: str) -> dict:
    """Snapshot all mutable org state so it can be restored after tests."""
    counters = session.execute(text(
        "SELECT * FROM usage_counters WHERE organization_id = :oid"
    ), {"oid": org_id}).fetchone()

    subscription = session.execute(text(
        "SELECT * FROM subscriptions WHERE organization_id = :oid"
    ), {"oid": org_id}).fetchone()

    return {
        "counters": dict(counters._mapping) if counters else None,
        "subscription": dict(subscription._mapping) if subscription else None,
    }


def _restore_org_state(session: Session, org_id: str, snapshot: dict):
    """Restore org state to pre-test snapshot."""
    if snapshot["counters"]:
        cols = snapshot["counters"]
        session.execute(text("""
            UPDATE usage_counters
            SET events_created = :events_created,
                synthesis_runs = :synthesis_runs,
                pack_builds = :pack_builds,
                memories_stored = :memories_stored,
                knowledge_items = :knowledge_items,
                active_anima_count = :active_anima_count,
                dormant_anima_count = :dormant_anima_count
            WHERE organization_id = :oid
        """), {
            "events_created": cols["events_created"],
            "synthesis_runs": cols["synthesis_runs"],
            "pack_builds": cols["pack_builds"],
            "memories_stored": cols["memories_stored"],
            "knowledge_items": cols["knowledge_items"],
            "active_anima_count": cols["active_anima_count"],
            "dormant_anima_count": cols["dormant_anima_count"],
            "oid": org_id,
        })

    if snapshot["subscription"]:
        cols = snapshot["subscription"]
        session.execute(text("""
            UPDATE subscriptions
            SET spending_cap_cents = :spending_cap_cents,
                plan_tier = :plan_tier,
                status = :status,
                is_internal = :is_internal
            WHERE organization_id = :oid
        """), {
            "spending_cap_cents": cols["spending_cap_cents"],
            "plan_tier": cols["plan_tier"],
            "status": cols["status"],
            "is_internal": cols["is_internal"],
            "oid": org_id,
        })
...
def reset_test_state_before_tests(test_context: dict):
    """
    Snapshot org state, clean stale test data before tests, restore after.
    ...
    and resets counters so RequireActionAllowed doesn't 403.

    After all tests, restores usage counters and subscription to their
    pre-test values so the prod DB is completely untouched.

    CASCADE FKs handle all child tables automatically.
    """
    session = get_admin_session()
    ...
    snapshot = _snapshot_org_state(session, org_id)

    try:
        # Purge stale test data (by user_id + org_id + NULL user_id orphans)
        ...
        session.execute(text("DELETE FROM animas WHERE organization_id = :oid"), {"oid": org_id})
        session.execute(text("DELETE FROM animas WHERE user_id IS NULL"))

        # Zero ALL counters for clean test baseline
        session.execute(text("""
            UPDATE usage_counters
            SET active_anima_count = 0, dormant_anima_count = 0,
                events_created = 0, synthesis_runs = 0,
                pack_builds = 0
            WHERE organization_id = :oid
        """), {"oid": org_id})

        # Ensure spending cap doesn't block tests
        session.execute(text("""
            UPDATE subscriptions
            SET spending_cap_cents = -1
            WHERE organization_id = :oid
        """), {"oid": org_id})

        session.commit()
        print(f"\n[SETUP] Stale test data purged + counters zeroed for org {org_id}")
    ...
    session = get_admin_session()
    try:
        # Delete all test-created data (by user_id + org_id + NULL user_id orphans)
        session.execute(text("DELETE FROM api_keys WHERE user_id = :uid"), {"uid": user_id})
        session.execute(text("DELETE FROM animas WHERE user_id = :uid"), {"uid": user_id})
        session.execute(text("DELETE FROM animas WHERE organization_id = :oid"), {"oid": org_id})
        session.execute(text("DELETE FROM animas WHERE user_id IS NULL"))
        session.execute(text("DELETE FROM billing_events WHERE organization_id = :oid"), {"oid": org_id})

        # Restore counters and subscription to exact pre-test state
        _restore_org_state(session, org_id, snapshot)

        session.commit()
        print(f"\n[CLEANUP] Test data deleted + org state restored for {test_context['email']}")
    except Exception as e:
        session.rollback()
        print(f"\n[CLEANUP ERROR] {e}")
    finally:
...
def _cleanup_anima(anima_id: str, org_id: str = None):
    """Delete a single anima — CASCADE FKs handle all child records.
