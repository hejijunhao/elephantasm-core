...
class TestZeroEventsSchedulerIntegration:
    ...
    @pytest.mark.asyncio
    async def test_execute_for_inactive_anima_no_crash(self):
        """
        ...
        try:
            ...
            with get_db_session() as session:
                ...
                config_obj = SynthesisConfigOperations.get_or_create_default(session, anima_id)
                session.delete(config_obj)
                session.flush()

                # Delete anima
                anima_obj = AnimaOperations.get_by_id(session, anima_id)
                if anima_obj:
                    session.delete(anima_obj)
