...
Tests that workflow steps enforce multi-tenant security:
...
from app.workflows.memory_synthesis.nodes.memory_persistence import persist_memory
from app.workflows.memory_synthesis.nodes.event_collection import collect_pending_events
from app.workflows.memory_synthesis.nodes.threshold_gate import check_synthesis_threshold
...
class TestMemoryPersistenceRLS:
    """Tests for RLS enforcement in memory persistence step."""
    ...
    def test_atomic_rollback_on_error(self, test_users, test_animas, test_events):
        ...
        # Execute with invalid input (missing llm_response)
...
class TestEventCollectionRLS:
    """Tests for RLS enforcement in event collection step."""
    ...
    def test_collects_only_user_events(self, test_users, test_animas, test_events):
        ...
        pending_events = collect_pending_events(anima1.id)
...
class TestThresholdGateRLS:
    """Tests for RLS enforcement in threshold gate step."""
    ...
    def test_reads_correct_user_config(self, test_users, test_animas):
        ...
        with session_with_rls_context(user1.id) as session:
            ...
            SynthesisConfigOperations.update(
                session,
                anima1.id,
                SynthesisConfigUpdate(threshold=49.0)
            )

        # Check threshold gate
        result = check_synthesis_threshold(
            anima_id=anima1.id,
...
class TestCrossUserIsolation:
    ...
    def test_user_cannot_trigger_synthesis_for_other_user_anima(self, test_users, test_animas, test_events):
        ...
        with session_with_rls_context(user1.id) as session:
            ...
            # Document existing behavior — event ownership validation is a future enhancement
            pass
...
class TestEdgeCases:
    ...
    def test_orphaned_anima_raises_error(self, rls_session: Session, test_user_context):
        ...
        with pytest.raises(ValueError, match="not found"):
            collect_pending_events(orphaned.id)
    ...
    def test_invalid_anima_id_raises_error(self):
        ...
        with pytest.raises(ValueError, match="not found"):
            collect_pending_events(fake_anima_id)
