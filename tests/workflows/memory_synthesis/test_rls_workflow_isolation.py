...
@pytest.fixture
def test_animas(db_session: Session, test_users):
    """Create test animas for each user."""
    user1, user2 = test_users

    anima1_data = AnimaCreate(name="User 1 Anima")
    anima1 = AnimaOperations.create(db_session, anima1_data, user_id=user1.id)

    anima2_data = AnimaCreate(name="User 2 Anima")
    anima2 = AnimaOperations.create(db_session, anima2_data, user_id=user2.id)
...
@pytest.fixture
def test_events(db_session: Session, test_animas):
    """Create test events for animas."""
    anima1, anima2 = test_animas

    # Events for anima1
    with session_with_rls_context(anima1.user_id) as session:
    ...
    # Events for anima2
    with session_with_rls_context(anima2.user_id) as session:
...
class TestEdgeCases:
    ...
    def test_orphaned_anima_raises_error(self, db_session: Session):
        """Test that orphaned anima (no user_id) raises error."""
        from app.models.database.animas import Anima

        # Create orphaned anima (bypass domain layer)
        orphaned = Anima(name="Orphaned Anima", user_id=None)
