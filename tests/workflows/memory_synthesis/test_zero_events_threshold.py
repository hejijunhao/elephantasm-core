...
class TestThresholdCheckEdgeCases:
    ...
    def test_negative_event_count_treated_as_zero(self, anima):
        ...
        # Note: Current implementation doesn't explicitly handle negative
        # but Python's truthiness treats -5 as truthy, so this would proceed
        # This test documents current behavior (may want to add validation)
        assert result["synthesis_triggered"] is True
        assert result["skip_reason"] is None

    def test_custom_anima_threshold(self):
        """
        ...
        """
        with get_db_session() as session:
            # Create anima with custom threshold
            anima = Anima(name="Custom Threshold Anima")
            session.add(anima)
            session.flush()
