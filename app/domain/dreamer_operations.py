...
class DreamerOperations:
    ...
    def _record_action(
        ...
        if action_type == DreamActionType.MERGE:
            dream_session.memories_created += len(result_memory_ids) if result_memory_ids else 1
    ...
    def merge_memories(
        ...
        """
        Merge N source memories into M new memories.

        Supports both pairwise merge (N→1) and cluster consolidation (N→M).
        Creates new memories with merged_from provenance in meta.
        ...
        Args:
            ...
            merged_definitions: List of dicts, each with:
                ...
                - content (str, optional): LLM-generated content
                - importance (float): 0.0-1.0
                - confidence (float): 0.0-1.0
                - source_indices (list[int], optional): indices into source list
                  for time span inheritance. If absent, uses all sources.
        ...
        if not merged_definitions:
            raise ValueError("Need at least 1 merged definition")
        ...
        # Create M new memories from definitions
        new_memories: list[Memory] = []
        all_source_ids = [str(m.id) for m in sources]

        for defn in merged_definitions:
            # Determine time span from source subset
            source_indices = defn.get("source_indices")
            if source_indices:
                subset = [
                    sources[i] for i in source_indices
                    if 0 <= i < len(sources)
                ] or sources
            else:
                subset = sources

            time_starts = [m.time_start for m in subset if m.time_start]
            time_ends = [m.time_end or m.time_start for m in subset if m.time_start]

            # Content: use LLM-provided content, fall back to concatenation for pairwise
            content = defn.get("content")
            if content is None:
                content = "\n\n---\n\n".join(
                    m.content for m in subset if m.content
                ) or None
