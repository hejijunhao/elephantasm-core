...
class EventOperations:
    ...
    def create(
        ...
        Raises HTTPException 404 (Anima not found) or 400 (validation failure).

        Pattern: Validate FK → apply defaults → create → add → flush.
        """
        ...
        if not anima or anima.is_deleted:
            raise HTTPException(
                status_code=404,
                detail=f"Anima {data.anima_id} not found or deleted"
            )
