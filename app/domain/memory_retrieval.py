...
class MemoryRetrieval:
    ...
    def get_pending_events(
        ...
        """
        Get events not yet synthesized into memories.

        Uses LEFT JOIN on memories_events to find events with no
        memory link. Ordered chronologically (ASC) for dialogue flow.
        Baseline timestamp narrows query window to events after last memory.

        Args:
            session: Database session
            anima_id: Anima to retrieve for
            limit: Max pending events to return
        ...
        # Baseline optimization: only scan events after last memory
        last_memory_time = MemoryOperations.get_last_memory_time(session, anima_id)
        if last_memory_time is None:
            anima = AnimaOperations.get_by_id(session, anima_id)
            baseline = anima.created_at if anima else datetime(2020, 1, 1, tzinfo=timezone.utc)
        else:
            baseline = last_memory_time

        if baseline.tzinfo is None:
            baseline = baseline.replace(tzinfo=timezone.utc)

        # LEFT JOIN anti-pattern: find events with no memory link
        result = session.execute(
            select(Event)
            ...
            .where(
                and_(
                    ...
                    MemoryEvent.id.is_(None),
                    Event.created_at >= baseline,
                )
            )
            .order_by(
                Event.occurred_at.asc().nulls_last(),
