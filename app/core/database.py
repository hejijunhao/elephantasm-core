...
# Cron engine — BYPASSRLS role for scheduled jobs
# Only used by scheduler orchestrator for system-level queries
# (listing all animas, stale session cleanup, etc.)
cron_engine = None
CronSessionLocal = None

if settings.CRON_DATABASE_URL:
    CRON_DB_URL = settings.CRON_DATABASE_URL
    if CRON_DB_URL.startswith("postgresql://"):
        CRON_DB_URL = CRON_DB_URL.replace("postgresql://", "postgresql+psycopg://", 1)

    cron_engine = create_engine(
        CRON_DB_URL,
        poolclass=NullPool,
        connect_args={
    ...
    cron_engine = cron_engine.execution_options(
        postgresql_prepared_statement_cache_size=0
    )

    @event.listens_for(cron_engine, "connect")
    def receive_cron_connect(dbapi_conn, connection_record):  # noqa: ARG001
        """DISCARD ALL on cron connections for pgBouncer compatibility."""
        try:
            cursor = dbapi_conn.cursor()
            cursor.execute("DISCARD ALL")
            dbapi_conn.commit()
    ...
    logger.info("Cron database engine configured (BYPASSRLS role)")
else:
    logger.warning(
        "CRON_DATABASE_URL not set — scheduler will use default engine "
        "(RLS will block system-level queries)"
    )


@contextmanager
def get_cron_db_session() -> Generator[Session, None, None]:
    """
    Database session for scheduled jobs (BYPASSRLS).

    Uses dedicated cron_service role that bypasses RLS entirely.
    ONLY for system-level operations: listing all animas, stale cleanup, etc.
    Falls back to SessionLocal if CRON_DATABASE_URL not configured.
    """
    factory = CronSessionLocal or SessionLocal
    session = factory()
    try:
        yield session
        session.commit()
...
def get_db_session() -> Generator[Session, None, None]:
    ...
    Used by workflow steps (sync operations in thread pool).
