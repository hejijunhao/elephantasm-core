...
from app.services.pack_persistence import build_pack_data, persist_pack_async
...
def _build_pack_response(pack) -> PackResponse:
    ...
    return PackResponse(
        ...
        has_pending_events=len(pack.pending_events) > 0,
        ...
        pending_events=[
            PendingEventResponse(
                ...
                occurred_at=pe.event.occurred_at or pe.event.created_at,
            )
...
async def compile_pack_with_preset(
    ...
    include_pending_events: Optional[bool] = Query(default=None, description="Override preset's pending events setting"),
    ...
    # Override pending events if explicitly provided
    if include_pending_events is not None:
        config.include_pending_events = include_pending_events
    ...
    if persist and background_tasks:
        pack_data = build_pack_data(pack, config, preset_name)
