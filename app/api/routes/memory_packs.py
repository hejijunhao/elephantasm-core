...
from app.models.database.memory_pack import MemoryPack, MemoryPackRead, MemoryPackStats
from app.services.memory_pack_compiler import MemoryPackCompiler
from app.services.pack_presets import get_conversational_preset, get_preset
from app.services.pack_persistence import build_pack_data
...
async def get_latest_memory_pack(
    anima_id: UUID,
    query: Optional[str] = Query(default=None, description="Semantic search query for context-aware compilation"),
    preset: Optional[str] = Query(default=None, description="Compilation preset (conversational or self_determined)"),
    db: Session = Depends(get_db_with_rls)
) -> Optional[MemoryPackRead]:
    """
    Get latest memory pack for anima, with smart compilation.

    Three modes:
    - **query provided** → always compiles fresh with pgVector semantic search
    - **no query, pack exists** → returns cached pack (fast lookup)
    - **no query, no pack** → compiles generic pack on-the-fly (compile-on-miss)

    Both SDKs already send `query` and `preset` as query params.
    """
    if query:
        # Query provided → always compile fresh (semantic results vary by query)
        preset_name = preset or "conversational"
        logger.info(f"Query-aware compile for anima {anima_id}, preset={preset_name}")
        config = await get_preset(preset_name, anima_id, query)
        compiled = MemoryPackCompiler(db).compile(config)

        pack_data = build_pack_data(compiled, config, preset_name)
        memory_pack = MemoryPack(**pack_data)
        saved = MemoryPackOperations.create(db, memory_pack)
        return MemoryPackRead.model_validate(saved)

    # No query → try cache first
    pack = MemoryPackOperations.get_latest(db, anima_id)
    if pack:
        return MemoryPackRead.model_validate(pack)

    # Compile-on-miss: build generic pack from current DB state
    logger.info(f"Compile-on-miss for anima {anima_id}")
    config = get_conversational_preset(anima_id)
    compiled = MemoryPackCompiler(db).compile(config)

    pack_data = build_pack_data(compiled, config, "conversational")
    memory_pack = MemoryPack(**pack_data)
    saved = MemoryPackOperations.create(db, memory_pack)
    return MemoryPackRead.model_validate(saved)
