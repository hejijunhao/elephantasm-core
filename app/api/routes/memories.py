...
@router.get("/map", response_model=List[MemoryMapPoint])
def get_memory_map(
    anima_id: UUID,
    limit: int = Query(300, ge=1, le=300, description="Max memories to project (1-300)"),
    force: bool = Query(False, description="Clear cached coordinates and recompute via UMAP"),
    db: Session = Depends(get_db_with_rls),
) -> List[MemoryMapPoint]:
    """
    Get UMAP-projected 2D coordinates for memory constellation visualization.
    Returns memories with (x, y) in [0, 1] range, positioned by semantic similarity.
    Uses cached coordinates when available; recomputes via UMAP on cache miss.
    Pass force=true to clear cache and trigger full UMAP recomputation.
    """
    if force:
        MemoryOperations.clear_map_coords(db, anima_id)
    return MemoryOperations.get_memory_map(db, anima_id, limit=limit)
...
async def get_memory_stats(
    ...
    if state:
        ...
        # All state counts in one query (GROUP BY instead of 4 round-trips)
        counts = MemoryOperations.count_all_states(db, anima_id)
        return MemoryStatsResponse(
            anima_id=anima_id,
            total=sum(counts.values()),
            active=counts.get(MemoryState.ACTIVE, 0),
            decaying=counts.get(MemoryState.DECAYING, 0),
            archived=counts.get(MemoryState.ARCHIVED, 0),
        )
