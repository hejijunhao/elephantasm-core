...
async def get_dream_stats(
    ...
    sessions = db.exec(
        select(DreamSession).where(DreamSession.anima_id == anima_id)
    ).all()

    total = len(sessions)
    completed = sum(1 for s in sessions if s.status == DreamStatus.COMPLETED)
    failed = sum(1 for s in sessions if s.status == DreamStatus.FAILED)
    running = sum(1 for s in sessions if s.status == DreamStatus.RUNNING)
    ...
    completed_sessions = [s for s in sessions if s.status == DreamStatus.COMPLETED]
    total_reviewed = sum(s.memories_reviewed for s in completed_sessions)
    total_modified = sum(s.memories_modified for s in completed_sessions)
    total_created = sum(s.memories_created for s in completed_sessions)
    total_archived = sum(s.memories_archived for s in completed_sessions)
    total_deleted = sum(s.memories_deleted for s in completed_sessions)
    ...
    return {
        ...
        "last_dream_at": (
            last_dream.completed_at.isoformat() if last_dream else None
        ),
        "aggregate_metrics": {
