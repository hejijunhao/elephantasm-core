...
async def _trigger_auto_knowledge_synthesis_async(memory_id: str) -> None:
    ...
    """
    try:
        from app.workflows.knowledge_synthesis import run_knowledge_synthesis

        logger.info(f"Triggering Knowledge Synthesis for Memory {memory_id}")

        # Execute pipeline (await completion)
        result = await run_knowledge_synthesis(memory_id)
        ...
        if error:
            ...
            # Trigger meditation check (counter increment + threshold gate)
            # Wrapped in to_thread() because trigger_auto_meditation_check is sync
            # (opens DB sessions, acquires advisory locks) and we're in an async context
            if created_count > 0 and result.anima_id:
