...
def run_deep_sleep(
    ...
    """
    Execute Deep Sleep phase (sync wrapper for async operations).
    ...
    """
    # All call paths reach here from a plain thread (no running event loop):
    # - Scheduler: asyncio.to_thread() -> _execute_dream_sync() -> run_dream()
    # - Background task: Starlette thread pool -> run_dream_background() -> run_dream()
    return asyncio.run(
        _run_deep_sleep_async(
            session, dream_session, context, light_results, config
        )
    )


async def _run_deep_sleep_async(
    session: Session,
    dream_session: DreamSession,
    context: DreamContext,
    light_results: LightSleepResults,
    ...
    """
    Async implementation of Deep Sleep phase.

    Processes merge candidates and review candidates sequentially
    to avoid overwhelming the LLM API and maintain transaction integrity.
    """
    ...
    if light_results.merge_candidates:
        ...
        for merge_group in light_results.merge_candidates:
            ...
            try:
                merged = await _process_merge_group(
                    session,
                    dream_session,
                    merge_group,
                    context,
    ...
    if review_ids:
        ...
        if memories_to_review:
            ...
            review_results = await _process_review_batch(
                session,
                dream_session,
                memories_to_review,
                context,
...
async def _process_merge_group(
    session: Session,
    dream_session: DreamSession,
...
async def _process_review_batch(
    ...
    for i in range(0, len(memories), batch_size):
        ...
        try:
            batch_results = await _review_memory_batch(
                session,
                dream_session,
                batch,
                context,
...
async def _review_memory_batch(
    session: Session,
    dream_session: DreamSession,
    memories: list[Memory],
    context: DreamContext,
