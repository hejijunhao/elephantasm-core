...
def run_deep_sleep(
    ...
    # Get LLM clients — merge and review use different models
    try:
    ...
    if light_results.merge_candidates:
        ...
        for merge_group in light_results.merge_candidates:
            ...
            try:
                ...
                error_msg = f"Merge failed for group {merge_group}: {type(e).__name__}: {e}"
    ...
    if light_results.merge_candidates:
        ...
        context.memories = list(
            session.exec(
                select(Memory).where(
                    Memory.id.in_(memory_ids), Memory.is_deleted.is_(False)
                )
            ).all()
        )
