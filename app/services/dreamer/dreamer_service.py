...
class DreamerService:
    ...
    def _generate_summary(
        ...
        # Report cluster consolidation and pairwise merges separately
        if deep_results:
        ...
        if deep_results:
            ...
            # Persist error details in config_snapshot for post-mortem diagnosis
            if deep_results.errors:
                parts.append(
                    f"Encountered {len(deep_results.errors)} non-fatal errors"
                )
                snapshot = dict(dream_session.config_snapshot or {})
                snapshot["deep_sleep_errors"] = deep_results.errors
                dream_session.config_snapshot = snapshot
                self.session.flush()
