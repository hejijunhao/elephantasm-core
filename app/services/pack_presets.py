...
async def get_self_determined_preset(
    ...
    llm = get_llm_client(provider="anthropic", model="claude-haiku-4-5-20251001")
...
Respond with JSON only, no explanation.'''
    ...
    return RetrievalConfig(
        ...
        include_pending_events=SELF_DETERMINED_FIXED["include_pending_events"],
        max_pending_events=SELF_DETERMINED_FIXED["max_pending_events"],
...
async def get_preset(
    ...
    Raises:
        DomainValidationError: If preset_name not recognized or query missing for self_determined
    """
    if preset_name == "conversational":
        return get_conversational_preset(anima_id, query)
    elif preset_name == "self_determined":
        if not query:
            raise DomainValidationError("self_determined preset requires a query")
