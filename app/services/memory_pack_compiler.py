...
class CompiledPack:
    ...
    # Four layers + pending events + temporal awareness
    ...
    pending_events: List[PendingEvent]  # Layer 2.5: unsynthesized events
    ...
    def to_prompt_context(self) -> str:
        ...
        # Pending events section (Layer 2.5: unsynthesized raw interactions)
        if self.pending_events:
            lines = []
            for pe in self.pending_events:
                ts = pe.event.occurred_at or pe.event.created_at
                event_type_str = pe.event.event_type.value if hasattr(pe.event.event_type, 'value') else str(pe.event.event_type)
                role = pe.event.role or event_type_str
                lines.append(f"[{ts.strftime('%Y-%m-%d %H:%M')}] {role}: {pe.event.content}")
            pending_text = "\n".join(lines)
            sections.append(
                f"## Recent Activity (Pending Synthesis)\n"
                f"The following recent interactions have not yet been consolidated into long-term memory:\n\n"
                f"{pending_text}"
            )
...
class MemoryPackCompiler:
    ...
    def compile(
        ...
        # Layer 2.5: Pending events (unsynthesized, chronological)
        pending_events = self._retrieve_pending_events(config)
        ...
        session_memories, pending_events, knowledge, long_term_memories = self._enforce_token_budget(
            session_memories, pending_events, knowledge, long_term_memories, identity, config.max_tokens
        )

        # Compute total tokens
        token_count = self._estimate_tokens(
            session_memories, pending_events, knowledge, long_term_memories, identity
        )
    ...
    def _retrieve_pending_events(
        self,
        config: RetrievalConfig,
    ) -> List[PendingEvent]:
        """
        Layer 2.5: Pending events (chronological, no scoring).

        Retrieves unsynthesized events — raw interactions not yet
        promoted to memories by the synthesis pipeline.
        """
        if not config.include_pending_events:
            return []

        events = MemoryRetrieval.get_pending_events(
            self.session,
            anima_id=config.anima_id,
            limit=config.max_pending_events,
        )

        return [PendingEvent(event=e) for e in events]
    ...
    def _enforce_token_budget(
        ...
        Priority order: Identity > Session > Pending Events > Knowledge > Long-term
        """
        # Reserve tokens for identity (fixed cost)
        identity_tokens = 150 if identity else 0
        remaining = max_tokens - identity_tokens

        # Budget split: 20% session, 15% pending, 30% knowledge, 35% long-term
        session_budget = int(remaining * 0.20)
        pending_budget = int(remaining * 0.15)
        knowledge_budget = int(remaining * 0.30)
        longterm_budget = int(remaining * 0.35)

        # Trim each layer
        trimmed_session = self._trim_memories_to_budget(session_memories, session_budget)
        trimmed_pending = self._trim_pending_to_budget(pending_events, pending_budget)
        ...
        return trimmed_session, trimmed_pending, trimmed_knowledge, trimmed_longterm
    ...
    def _trim_pending_to_budget(
        self,
        pending: List[PendingEvent],
        budget: int,
    ) -> List[PendingEvent]:
        """Trim pending events to fit token budget."""
        trimmed = []
        tokens_used = 0
        for pe in pending:
            est_tokens = len(pe.event.content or "") // 4
            if tokens_used + est_tokens > budget:
                break
            trimmed.append(pe)
            tokens_used += est_tokens
    ...
    def _estimate_tokens(
        ...
        for pe in pending:
            total += len(pe.event.content or "") // 4
        for k in knowledge:
    ...
    def _serialize_pack(self, pack: CompiledPack) -> Dict[str, Any]:
        ...
        content = {
            ...
            "config": {
                ...
                "include_pending_events": pack.config.include_pending_events,
                "max_pending_events": pack.config.max_pending_events,
                "weights": {
