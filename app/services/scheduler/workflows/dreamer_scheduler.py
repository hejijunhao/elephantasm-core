...
from app.core.database import get_cron_db_session, get_db_session, get_db_with_rls_context
...
# Minimum seconds between execute_for_all_animas runs (prevents double-fire)
...
class DreamerScheduler(SchedulerBase):
    ...
    async def execute_for_anima(
        ...
        Cross-machine advisory lock prevents duplicate dreams. In-memory _running
        set is a fast-path local guard. All sync DB/dream work runs in a dedicated
        ...
        """
        # In-memory concurrency guard (fast-path local dedup)
        ...
        try:
            # Cross-machine advisory lock + sync execution in thread
            result = await asyncio.to_thread(
                self.execute_with_lock_sync,
                self._execute_dream_sync, anima_id, trigger_source,
                anima_id=anima_id,
            )
            if result is None:
                logger.info(f"Anima {anima_id}: Dream skipped — another machine holds the lock")
                return {
                    "success": True,
                    "skipped": True,
                    "anima_id": str(anima_id),
    ...
    async def execute_for_all_animas(self) -> dict[str, Any]:
        ...
        """
        # Cross-machine advisory lock (prevents both machines running sweep)
        result = await self.execute_with_lock(self._execute_all_animas_inner)
        if result is None:
            logger.info("Dream cycle skipped — another machine holds the lock")
            return {"skipped": True, "reason": "advisory_lock"}
        return result

    async def _execute_all_animas_inner(self) -> dict[str, Any]:
        """Inner sweep logic — runs under advisory lock."""
        run_start = datetime.now(timezone.utc)

        # Cooldown: skip if last run was too recent (prevents double-fire)
        if self._last_run:
            elapsed = (run_start - self._last_run).total_seconds()
            if elapsed < COOLDOWN_SECONDS:
                logger.warning(
                    f"Skipping dream cycle — cooldown ({elapsed:.0f}s < {COOLDOWN_SECONDS}s)"
                )
                return {"skipped": True, "reason": "cooldown", "elapsed_seconds": elapsed}
        ...
        # Uses cron session (BYPASSRLS) — dream_sessions has RLS enabled
        stale_recovered = 0
        ...
        # Fetch all active anima IDs — cron session bypasses RLS
        # (scheduled jobs have no user context; elephant role returns 0 rows)
        with get_cron_db_session() as session:
        ...
        # Aggregate statistics (mutually exclusive categories)
        exceptions = sum(1 for r in results if isinstance(r, Exception))
        if exceptions:
            logger.warning(
                f"{exceptions} anima(s) raised unhandled exceptions in dream cycle"
            )
        failed = exceptions + sum(
            1 for r in results if isinstance(r, dict) and not r.get("success")
        )
        skipped = sum(
            1 for r in results if isinstance(r, dict) and r.get("success") and r.get("skipped")
        )
        successful = sum(
            1 for r in results if isinstance(r, dict) and r.get("success") and not r.get("skipped")
        )
