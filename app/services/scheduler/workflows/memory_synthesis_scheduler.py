...
from app.core.database import get_cron_db_session, get_db_session
from app.workflows.memory_synthesis import run_memory_synthesis
...
class MemorySynthesisScheduler(SchedulerBase):
    ...
    async def execute_for_anima(
        ...
        """
        # Cross-machine advisory lock (authoritative guard)
        result = await self.execute_with_lock(
            self._execute_for_anima_inner, anima_id, trigger_source,
            anima_id=anima_id,
        )
        if result is None:
            logger.info(f"Anima {anima_id}: Synthesis skipped — another machine holds the lock")
            return {
                "success": True,
                "skipped": True,
    ...
    async def _execute_for_anima_inner(
        self, anima_id: UUID, trigger_source: str = "scheduled"
    ) -> Dict[str, Any]:
        """Inner synthesis logic — runs under advisory lock."""
        ...
        try:
            # Execute pipeline
            result = await run_memory_synthesis(str(anima_id))

            # Check for pipeline error (e.g., anima not found, DB failure)
            if result.error:
                logger.error(f"Anima {anima_id}: Synthesis error - {result.error}")
                return {
                    "success": False,
                    "skipped": False,
                    "anima_id": str(anima_id),
            ...
            if not result.synthesis_triggered:
    ...
    async def execute_for_all_animas(self) -> Dict[str, Any]:
        ...
        if result is None:
            logger.info("Memory synthesis sweep skipped — another machine holds the lock")
    ...
    async def _execute_all_animas_inner(self) -> Dict[str, Any]:
        ...
        if exceptions:
            logger.warning(
                f"{exceptions} anima(s) raised unhandled exceptions in synthesis cycle"
            )
        failed = exceptions + sum(
            1 for r in results if isinstance(r, dict) and not r.get("success")
        )
        skipped = sum(1 for r in results if isinstance(r, dict) and r.get("success") and r.get("skipped"))
