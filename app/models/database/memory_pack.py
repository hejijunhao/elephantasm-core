...
class MemoryPackBase(SQLModel):
    ...
    pending_event_count: int = Field(
        default=0,
        description="Number of pending events included",
    )
    ...
    #   "pending_events": [{ id, content, summary, event_type, role, author, occurred_at }],
...
class MemoryPackStats(SQLModel):
    ...
    avg_pending_events: float = Field(description="Average pending events per pack")
