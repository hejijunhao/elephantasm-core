...
class SynthesisConfigBase(SQLModel):
    ...
    anima_id: UUID = Field(
        sa_column=Column(
            ForeignKey("animas.id", ondelete="CASCADE"),
    ...
    # Meditation trigger counter
    knowledge_synth_count: int = Field(default=0, ge=0, description="Knowledge syntheses since last meditation (reset on meditation)")
    meditation_threshold: int = Field(default=10, ge=1, le=100, description="Meditate after N knowledge syntheses (1-100)")
...
class SynthesisConfigRead(SynthesisConfigBase):
    ...
    knowledge_synth_count: int = 0
    meditation_threshold: int = 10
    created_at: datetime
    updated_at: datetime
...
class SynthesisConfigUpdate(SQLModel):
    ...
    meditation_threshold: int | None = Field(None, ge=1, le=100)
