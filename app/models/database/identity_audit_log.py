...
class IdentityAuditLog(SQLModel, table=True):
    ...
    identity_id: UUID = Field(
        sa_column=Column(ForeignKey("identities.id", ondelete="CASCADE"), nullable=False, index=True),
    )
    ...
    source_memory_id: Optional[UUID] = Field(
        default=None,
        sa_column=Column(ForeignKey("memories.id", ondelete="SET NULL"), nullable=True, index=True),
    )
