...
class AnimaBase(SQLModel):
    ...
    user_id: UUID | None = Field(
        ...
        description="Owner user reference (RLS filtering)",
    )
    organization_id: UUID | None = Field(
        default=None,
        sa_column=Column(ForeignKey("organizations.id", ondelete="SET NULL"), nullable=True, index=True),
        description="Owning organization (org-scoped access)",
    )
