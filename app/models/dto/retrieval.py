...
class RetrievalConfig(BaseModel):
    ...
    include_pending_events: bool = Field(
        default=True,
        description="Include unsynthesized events in pack (Layer 2.5)",
    )
    max_pending_events: int = Field(
        default=20,
        ge=0,
        le=50,
        description="Max pending events to include",
    )
