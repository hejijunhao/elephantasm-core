...
def persist_memory(
    ...
    with session_with_rls_context(user_id) as session:
        ...
        # Invalidate UMAP cache — new memory means constellation must reproject
        _clear_map_cache(session, anima_id)
...
def _clear_map_cache(session, anima_id: UUID) -> None:
    """Clear UMAP constellation cache for anima (best-effort)."""
    try:
        cleared = MemoryOperations.clear_map_coords(session, anima_id)
        if cleared:
            logger.info(f"Cleared UMAP cache for anima {anima_id} ({cleared} memories)")
    except Exception as e:
        logger.warning(f"Failed to clear UMAP cache for anima {anima_id}: {e}")
