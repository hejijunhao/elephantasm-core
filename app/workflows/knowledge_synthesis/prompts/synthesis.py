...
def _format_memory_for_prompt(memory: Dict[str, Any]) -> str:
    """
    Format Memory data for inclusion in prompt.

    Helper function for future extensions (e.g., including source events).

    Args:
        memory: Serialized Memory dict

    Returns:
        Formatted text representation
    """
    summary = memory.get('summary', '')
    content = memory.get('content') or summary
